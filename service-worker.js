...
const CACHE_VERSION = 'mobile-v2.0';
const STATIC_CACHE = `novelist-tools-static-${CACHE_VERSION}`;
const DYNAMIC_CACHE = `novelist-tools-dynamic-${CACHE_VERSION}`;
...
async function cleanupOldCaches() {
  ...
  const oldCaches = cacheNames.filter(name =>
    name.includes('novelist-tools') && !name.includes(CACHE_VERSION)
  );
